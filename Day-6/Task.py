...
def main():
    ...
    hamza = Library(list_books, Library_name)

    print(f"\t\tWelecome To {hamza.library_name} library\n\ne for exit \nDisplay Book Using 's' \nadd lend book using 'l'\nReturn a Book using 'r' \nAdd Book Using 'a' \nDelete Book using 'd' \n ")
    Exit = False
    while (Exit is not True):
        ...
        if _input == "e":
            ...
            hamza.lend_book(_input3, _input2)

        elif _input == "a":
            _input2 = input("Book name:")
            hamza.add_book(_input2)
            ...
            hamza.return_book(_input3, _input2)
